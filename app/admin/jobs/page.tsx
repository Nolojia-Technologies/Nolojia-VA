    ...
    <div>
      ...
      <div className="p-6 space-y-5">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
              ...
              <Link
                ...
                className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                  (s === 'all' && !searchParams.status) || searchParams.status === s
                    ? 'bg-brand text-white'
                    : 'bg-card text-muted-foreground border border-border hover:bg-surface-2'
                }`}
              >
          ...
          <Link
            href="/admin/jobs/new"
            className="flex items-center gap-2 bg-brand hover:bg-brand-hover text-white px-4 py-2 rounded-xl text-sm font-medium transition-colors shadow-sm"
          >
        ...
        {jobs && jobs.length > 0 ? (
          <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-4">
            {jobs.map((job) => (
              <div
                key={job.id}
                className="bg-card rounded-2xl border border-border shadow-sm hover:shadow-md transition-shadow p-5 flex flex-col"
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="w-10 h-10 rounded-xl bg-brand-soft flex items-center justify-center">
                    <Briefcase aria-hidden="true" size={18} className="text-brand" />
                  </div>
                ...
                <div className="flex items-center gap-2">
                  <Link
                    ...
                    className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg bg-surface hover:bg-muted text-foreground/85 text-xs font-medium transition-colors"
                  >
                  ...
                  <Link
                    ...
                    className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg bg-brand-soft hover:bg-brand-soft text-brand text-xs font-medium transition-colors"
                  >
                    <Users aria-hidden="true" size={13} />
                    View Applicants
                  </Link>
          ...
          <div className="bg-card rounded-2xl border border-border shadow-sm flex flex-col items-center justify-center py-20 text-center">
            <Briefcase aria-hidden="true" className="w-12 h-12 text-muted-foreground/60 mb-3" />
            <h3 className="text-base font-semibold text-foreground/85">No job listings yet</h3>
            <p className="text-sm text-muted-foreground mt-1 mb-5">Create your first job listing to start receiving applications</p>
            <Link
              href="/admin/jobs/new"
              className="flex items-center gap-2 bg-brand text-white px-5 py-2.5 rounded-xl text-sm font-medium hover:bg-brand-hover transition-colors"
            >
              <Plus aria-hidden="true" size={16} />
              Create Job Listing
            </Link>
